...
class Store:
    ...
    def add(self, record: Record) -> Record:
        ...
        with self.connection:
            self.connection.execute(
                "INSERT INTO restitution_case(record_id, owner_id, state, revision, created_at) VALUES(?,?,?,?,?)",
    ...
    def get(self, record_id: str) -> Record | None:
        row = self.connection.execute(
            "SELECT record_id, owner_id, state, revision, created_at FROM restitution_case WHERE record_id=?",
            (record_id,),
        ).fetchone()
